...
def to_csv(asks, bids, times):
    ...
    res.to_csv("BTCUSDT_SPOT_ORDERBOOK_DIFF_2021_08_WOO.csv", header=False, index=False, mode="a")
...
def to_csv_10(asks, bids, time):
    ...
    df.to_csv("BTCUSDT_SPOT_ORDERBOOK_FULL_2021_08_WOO.csv", header=False, index=False, mode="a")

def init():
    res = pd.concat([get_lists([[]], [[]], "asks"), get_lists([[]], [[]], "bids")], axis=1)
    res.to_csv("BTCUSDT_SPOT_ORDERBOOK_DIFF_2021_08_WOO.csv", header=True, index=False, mode="w")
    
def init_interval():
    data = {'timestamp': [], 'data':[]}
    res_10 = pd.DataFrame(data)
    res_10.to_csv("BTCUSDT_SPOT_ORDERBOOK_FULL_2021_08_WOO.csv", header=True, index=False, mode="w")
