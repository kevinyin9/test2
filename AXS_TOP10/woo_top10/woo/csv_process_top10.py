...
def to_csv(asks, bids, times):
    ...
    res.to_csv("AXSUSDT_SPOT_ORDERBOOK_TOP10_10.csv", header=False, index=False, mode="a")
...
def to_csv_10(asks, bids, time):
    ...
    df.to_csv("AXSUSDT_SPOT_ORDERBOOK_TOP10_10.csv", header=False, index=False, mode="a")

def init():
    res = pd.concat([get_lists([[]], [[]], "asks"), get_lists([[]], [[]], "bids")], axis=1)
    res.to_csv("AXSUSDT_SPOT_ORDERBOOK_TOP10_10.csv", header=True, index=False, mode="a")
    
def init_interval():
    data = {'timestamp': [], 'data':[]}
    res_10 = pd.DataFrame(data)
    res_10.to_csv("AXSUSDT_SPOT_ORDERBOOK_TOP10_10.csv", header=True, index=False, mode="a")
...
def write_interval(csv_asks, cvs_bids, csv_since):
    t2 = threading.Thread(target = to_csv_10,  args=[csv_asks[-1], cvs_bids[-1], csv_since[-1]])
    t2.start()
